class notion_heading_2_block : notion_heading_block
{
    ...
    notion_heading_2_block($text) : base("heading_2")
    {
        $this.heading_2 = [Heading_structure]::new($text)
    }

    notion_heading_2_block($text, $color) : base("heading_2")
    {
        $this.heading_2 = [Heading_structure]::new($text, $color)
    }

    notion_heading_2_block($text, [bool] $is_toggleable) : base("heading_2")
    {
        $this.heading_2 = [Heading_structure]::new($text, $is_toggleable)
    }

    notion_heading_2_block($text, $color, [bool] $is_toggleable) : base("heading_2")
    {
